function excelDateToJSDate(serial) {
  if (typeof serial !== 'number' || serial < 1) return serial; // if not number or invalid, return as is
  const excelEpoch = new Date(1900, 0, 1); // Jan 1, 1900
  const days = serial - 1; // Excel starts from 1
        ...
        if (row[0] && !row[0].toString().trim().startsWith(' ')) {
          ...
          currentPos = {
            ...
            "Aggregate Position": [{
              ...
              "Ex-Div Date": excelDateToJSDate(row[14]),
              "Dividend Pay Date": excelDateToJSDate(row[15]),
              "Dividend": row[16]
            }],
            "Transaction Data": []
          };
